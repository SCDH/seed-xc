...
public class Service implements DefaultApi {
	...
	public Response compileZip(String stylesheet, File body) {
		try {
			...
			// we use RestResponse because it makes the error message occur in
			// response body
			RestResponse<String> response = RestResponse.status(
				Response.Status.NOT_IMPLEMENTED, e.getMessage());
