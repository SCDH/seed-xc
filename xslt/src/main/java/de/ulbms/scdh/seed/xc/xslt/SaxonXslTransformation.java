...
public class SaxonXslTransformation implements Transformation {
	...
	LOG.debug("Setting up new SaxonXslTransformation with identifier '{}' ...",
	...
	try {
		...
		for(TransformationInfoLibrariesInner library : transformationInfo.getLibraries()) {
			...
			throw new ConfigurationException("Failed to compile package from '"
							 + library.getLocation()
							 + "': " + e.getMessage(),
							 e);
		    }
	    ...
	    LOG.error("Failed to setup transformation '{}':\n{}",
		      transformationInfo.getIdent(),
		      e.getMessage());
	    throw new ConfigurationException("failed to setup transformation '"
					     + transformationInfo.getIdent()
					     + "': " + e.getMessage(), e);
	...
	for (QName name : parameterMap.keySet()) {
	    ...
	    try {
		description.setOccurrenceIndicator(details.getDeclaredCardinality().toString());
	    } catch (NullPointerException e) {
		LOG.error("cardinality not declared value for parameter {} in transformation {}",
			  name, transformationInfo.getIdent());
	    }
	    try {
		description.setItemType(details.getDeclaredItemType().getTypeName().toString());
	    } catch (NullPointerException e) {
		LOG.error("item type not declared for parameter {} in transformation {}",
			  name, transformationInfo.getIdent());
	    }
	    try {
		description.setUnderlyingDeclaredType(details.getUnderlyingDeclaredType().toString());
	    } catch (NullPointerException e) {
		LOG.error("underlying item type not declared value for parameter {} in transformation {}",
			  name, transformationInfo.getIdent());
	    }
	    try {
		description.setIsRequired(details.isRequired());
	    } catch (NullPointerException e) {
		LOG.error("cannot determine if parameter {} in transformation {} is required or not",
			  name, transformationInfo.getIdent());
	    }
	...
	} else if (transformationInfo.getParser() != null && transformationInfo.getParser().getPropertyClass() != null) {
	...
	if (config != null && config.getParser() != null && config.getParser().getXinclude() != null) {
		...
		LOG.error("xinclude-aware parsing not supported by {}", parser.getClass().getCanonicalName());
		// throw new TransformationPreparationException("xinclude-aware parsing not supported by " + parser.getClass().getCanonicalName());
	    }
	}
     ...
     */
    protected synchronized void transform(RuntimeParameters parameters, Config config, Source source, Serializer serializer)
	throws TransformationPreparationException, TransformationException {
	...
	try {
	    ...
	    throw new TransformationPreparationException("failed to set up transformation parameters: " + e.getMessage(), e);
	}

	// TODO: evaluate evaluate initialTemplate and initialFunction from runtime parameters
	try {
     ...
     */
    protected Map<QName, XdmValue> makeStylesheetParameters(RuntimeParameters parameters) throws TransformationPreparationException, TransformationException {
	...
	for (QName name : executable.getGlobalParameters().keySet()) {
		...
		try {
		    LOG.debug("converting parameter '{}' ('{}') to {}", nameString, stringValue.toString(), itemType.getTypeName());
		    //ConversionRules conversionRules2 = itemType.getConversionRules();
		    AtomicType atomicType = (AtomicType) itemType.getUnderlyingItemType().getPrimitiveItemType(); // FIXME: what about sequence types
		    StringConverter converter = conversionRules.makeStringConverter(atomicType);
		    AtomicValue atomicValue = converter.convertString(StringView.of(parametersMap.get(nameString))).asAtomic();
		    XdmAtomicValue value = XdmAtomicValue.makeAtomicValue(atomicValue);
		    stylesheetParameters.put(name, value);
		} catch (ValidationException e) {
		    LOG.error("failed to convert '{}' parameter value '{}' to {}",
			      nameString,
			      parameters.getGlobalParameters().get(nameString),
			      itemType.getTypeName());
		    throw new TransformationPreparationException
			("failed to convert '" + nameString + "' parameter value '" + parameters.getGlobalParameters().get(nameString) + "' to " + itemType.getTypeName().toString(), e);
		} catch (NullPointerException e) {
		    LOG.error("failed to convert '{}' parameter value due to missing type declaration. Transformation '{}'",
