...
public class TransformService implements TransformApi {
	...
	@Override
	public Response transformTransformationUrlPost(
		...
		if (transformationsMap.containsKey(transformationId)) {
			try {
				...
				byte[] output = transformation.transform(parameters, config, url);
				RestResponse<byte[]> response;
				if (transformation.getOutputMediaType() != null) {
					response = RestResponse.ok(output, transformation.getOutputMediaType());
				} else {
				...
				return RestResponse.status(Status.BAD_REQUEST, e.getMessage()).toResponse();
			} catch (TransformationException e) {
				// the log must reflect the fact that
				// transformTransformationUrlGet is also handled here
				LOGGER.error(
						"failed GET /transform/" + transformationId + "/{url} (or POST with runtime parameters)", e);
				return RestResponse.status(Status.INTERNAL_SERVER_ERROR, e.getMessage())
						.toResponse();
			}
		} else {
	...
	@Override
	public Response transformTransformationUrlGet(String transformationId, String url) {
	...
	public Response transformTransformationPost(
		...
		if (transformationsMap.containsKey(transformationId)) {
			try {
				...
				byte[] output = transformation.transform(parameters, config, url, source);
				RestResponse<byte[]> response;
				if (transformation.getOutputMediaType() != null) {
					response = RestResponse.ok(output, transformation.getOutputMediaType());
				} else {
