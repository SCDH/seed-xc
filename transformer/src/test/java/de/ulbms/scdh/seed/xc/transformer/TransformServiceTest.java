...
public class TransformServiceTest {
	...
	private final File tagsoupHtml =
		Paths.get("src", "test", "resources", "samples", "tagsoup.html")
			.toFile();
	...
	public void testTransformParamIntegerPostNoParam() {
		given()
			...
			.body(is("transformation failed: No value supplied for required "
					 + "parameter times"));
	}
	...
	public void testTransformParamIntegerPost() {
		given()
			.contentType("multipart/form-data")
			.multiPart("source", helloXml, "application/xml")
			.multiPart("runtimeParameters",
					   "{\"globalParameters\":{\"times\":\"3\"}}",
					   "application/json")
			.when()
			.post("/transform/param-integer")
			.then()
